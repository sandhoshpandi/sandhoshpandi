...
public interface BarcodeRepository extends JpaRepository<Barcode, Long> {
	...
	@Query(value = "SELECT DistributorID,DistributorName,taxStatus,vendorcode from medc_distributor.medc_distributorinformation WHERE companyrefid=?1 and branchrefid=?2 and locname=?3 and locrefid=?4 and status=0", nativeQuery = true)

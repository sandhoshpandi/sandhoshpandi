...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
	...
	@Query(value = "SELECT emp.employeeID,cmp.companyname,brn.branchname,loc.locationname,concat(IFNULL(shp.shopname,''),IFNULL(whr.warehousename,''),IFNULL(hsp.hospitalname,''))AS name,emp.empfirstname,emplastname,ecd.empmobileno,ede.departmentname,emp.desgination,esde.subdepartmentname,edi.divisionname,esdi.subdivisionname,emp.roleid,emp.rolename,emp.roleranking  from medc_employee.medc_employeedetails emp "
	...
	@Query(value = "SELECT employeeID,cmp.companyname,brn.branchname,loc.locationname,concat(IFNULL(shp.shopname,''),IFNULL(whr.warehousename,''),IFNULL(hsp.hospitalname,''))AS name,emp.empfirstname,emplastname,emp.department,emp.division,emp.desgination,emp.roleid,emp.rolename,emp.roleranking  from medc_employee.medc_employeedetails emp "
...
//	String getEmpImage(@Param("empid") int empid);
	...
	@Query(value ="select roleranking from medc_adminsecurity.medc_role ro where ro.RoleID=?1", nativeQuery = true)
	int getRoleranking(Integer roleid);

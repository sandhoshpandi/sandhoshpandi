...
public interface DrugRepository extends JpaRepository<Drug, Integer> {
	...
	@Query(value = "select ProductDrugID,coalesce(BrandName,0),coalesce(GenericID,0),coalesce(GenericNameDosage,0),coalesce(UOM,0),coalesce(VAT,0),\n" + 
			...
			"coalesce(production_sunlight,'No'),coalesce(sanitizing,'No'),verticalid,coalesce(coldstorage,'No'),groupid,subgroupid1,subgroupid2,coalesce(pharmacompanyid,0),hsnid,skucode,dosageenglish,dosageburmese,packagesizeenglish,packagesizeburmese,sellingunitenglish,sellingunitburmese,actionindicationenglish,actionindicationburmese,productinformationenglish,productinformationburmese,potentialsideeffectsenglish,potentialsideeffectsburmese\n" + 
	...
	@Query(value = "SELECT pm.productdrugid,coalesce(pm.BrandName,'zin'),coalesce(gn.GenericName,'zsc'),pm.UniformProductCode,coalesce(pm.GenericNameDosage,'zen'),\n" + 
			"tm.VAT,pm.GST,coalesce(fo.FormulationName,'zof'),pm.mrp,pc.PCompanyName,hsn.hsncode,hsn.hsndescription,sch.schedulename,pm.skucode ,pm.dosageenglish,pm.dosageburmese,pm.packagesizeenglish,pm.packagesizeburmese,pm.sellingunitenglish,pm.sellingunitburmese,pm.actionindicationenglish,pm.actionindicationburmese,pm.productinformationenglish,pm.productinformationburmese,pm.potentialsideeffectsenglish,pm.potentialsideeffectsburmese FROM medc_productmaster.medc_custproductmaster pm\n" + 
			"LEFT JOIN medc_productmaster.medc_formulation fo on fo.FormulationID=pm.formulationid\n" + 
   ...
   @Query(value = "SELECT cpm.productdrugid,CONCAT(cpm.brandname,' ',cpm.genericnamedosage) brandname,f.formulationname,gm.genericname FROM medc_productmaster.medc_custproductmaster cpm\r\n" + 
	...
	@Transactional
	@Query(value = "update medc_productmaster.medc_custproductmaster set image_url =?1,image_filename=?3  where ProductDrugID=?2   ", nativeQuery = true)
	void uploadDrugImagePaths(String string, int updateid, String string2);

	
	@Query(value = "SELECT m.image_url FROM medc_productmaster.medc_custproductmaster m where m.ProductDrugID=?1 ", nativeQuery = true)
	String getsendImages(@Param("id") int id);

	
//	@Query(value = "SELECT m.image_url FROM medc_productmaster.medc_custproductmaster m where m.ProductDrugID=?1 ", nativeQuery = true)
//	String[] getsendImages1(@Param("id") int id);

	
	@Query(value = "SELECT m.image_filename FROM medc_productmaster.medc_custproductmaster m where m.ProductDrugID=?1 ", nativeQuery = true)
	String getimagefilenames(Integer drugid);
// view maha repository

	List<Drug> findByCompanyidAndBranchidAndLocnameAndLocrefidOrderByIdDesc(Integer companyid, Integer brnchid,
			Integer locname, Integer locrefid);

	
	
//	@Query( value = "SELECT * FROM medc_productmaster.medc_custproductmaster order by ProductDrugID DESC  " , nativeQuery = true)
//		
//	List<Drug> findByCompanyidAndBranchidAndLocnameAndLocrefid(Integer companyid, Integer brnchid, Integer locname,
//			Integer locrefid);

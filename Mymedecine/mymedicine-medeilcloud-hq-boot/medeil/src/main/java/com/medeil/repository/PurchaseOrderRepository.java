...
public interface PurchaseOrderRepository extends JpaRepository<PurchaseOrder, Long> {
	...
	@Query(value = "SELECT pp.drugproductrefid,pr.brandname,pr.genericnamedosage,pp.boxquantity,pp.stripquantity,pp.tabletquantity,"
			+ "pp.uom,pp.equalto,pp.Unitprice,pp.totalproductprice,pp.abc,pp.distprodrank,pp.distremarks,pp.pursessionid,pp.pursessionno,pp.poproductid,coalesce(pp.reorderlvl,0),coalesce(pp.consumpminqty,0),coalesce(pp.minleadtime,0),coalesce(pp.maxqty,0),pp.stripperbox,pp.quantityperstrip,pp.totalquantity,pp.packageunit,pp.PORefID,pp.Discount,pp.DisAmount,pp.`Status`,pp.foc,pp.ctamount,pp.ct,pp.disper,pp.TotalProductPrice,po.taxstatus "
			+ "FROM medc_purchase.medc_poproduct pp "
			+ "left join medc_purchase.medc_purchaseorder po on po.poid=pp.porefid "
			+ "left join  medc_productmaster.medc_custproductmaster pr on pr.productdrugid=pp.drugproductrefid "
	...
	@Query(value = "SELECT m.DistributorID,m.DistributorName,m.vendorcode,m.Email FROM medc_distributor.medc_distributorinformation m where m.DistributorID=?1", nativeQuery = true)
	...
	@Query(value ="SELECT pr.purcreqno,pr.status,pr.purchasereqid,pr.vendorname,pr.vendorcode,pr.vendorid,pr.taxstatus,pp.purcreqprodid FROM medc_purchase.medc_purchaserequisition pr INNER JOIN medc_purchase.medc_purchaserequisitionproduct pp on pr.purcreqno = pp.purcreqnoref where pr.status = 2 and pr.companyrefid = ?1 and pr.branchid = ?2 and pr.locname = ?3 and pr.locrefid = ?4 group by pp.purcreqnoref", nativeQuery = true)
	List purchaseprno(Integer comp, Integer brnch, Integer locname, Integer locrefid);
	
	//@Query(value = "SELECT po.POID FROM  medc_purchase.medc_purchaseorder po WHERE status != 5", nativeQuery = true)
    Optional<PurchaseOrder> findById(Integer id);


	@Query(value = "select ad.prevacstatus from medc_adminsecurity.medc_adduserpurchaseprev ad where ad.purmorefid=2 and ad.suserid=?1", nativeQuery = true)
	Integer PoStatus(Integer suserid);

	@Query(value = "SELECT * from medc_purchase.medc_purchaseorder po where po.companyrefid=?1  and po.BranchRefID=?2  and po.locname=?3  and po.locrefid=?4 and status!=5 and status != 2", nativeQuery = true)
    List<PurchaseOrder> findByCompanyrefidAndBranchidAndLocnameAndLocrefid(Integer compid, Integer brnchid, Integer loc,
			Integer locrefid);

...
public interface PurchReturnRepository extends JpaRepository<PurchaseReturn, Long> {
	...
	@Query(value = "SELECT p.PRID,p. PRNo,p.invoiceno,date(p.prdate),p.totalitems,p.totalqty,pr.TotalQuantity  \r\n" + 
			"FROM medc_purchasereturn.medc_dirpurchasereturn as p \r\n" + 
			"INNER JOIN medc_purchasereturn.medc_prproduct as pr on pr.PIRefId=p.PRID\r\n" + 
			"where  p.LocName=?1 and p.LocRefID=?2  and   p.Status!=5  order by  p.PRID desc ", nativeQuery = true)

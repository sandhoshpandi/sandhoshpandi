...
public interface PurchaseRepository extends JpaRepository<Purchase, Long>, PurcRepository {
	...
	@Query(value = "SELECT DistributorID,DistributorName,dist_invno FROM medc_distributor.medc_distributorinformation Where CompanyRefID= :cid and BranchRefID= :bid and LocRefID= :locrefid and LocName=3 and status=0", nativeQuery = true)

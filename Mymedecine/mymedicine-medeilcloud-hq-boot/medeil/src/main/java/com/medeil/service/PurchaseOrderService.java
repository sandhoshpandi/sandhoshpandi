...
//	}
   ...
   public List purchaseprno(Integer comp, Integer brnch, Integer locname, Integer locrefid) {
		return POrderRepository.purchaseprno(comp, brnch,locname, locrefid);
	}

	

	public ResponseEntity<?> viewPurchaseorderProductbyid(Integer porefid) {
		Optional<PurchaseOrder> po = POrderRepository.findById(porefid);
		return ResponseEntity.created(null).body(po);
	}
	
	public ResponseEntity<?> PoStatus(Integer suserid) {
		Integer status=POrderRepository.PoStatus(suserid);
	    return ResponseEntity.created(null).body(status);
		
	}

	public ResponseEntity<?> UpdatePoAcknowledge(Integer id, Integer ackid, Integer status) {
		Optional<PurchaseOrder> po=POrderRepository.findById(id);
		PurchaseOrder po1=po.get();
        po1.setAcknowledgedid(ackid);
	
		   DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");  
		   LocalDateTime now = LocalDateTime.now();  
		   System.out.println(dtf.format(now));  
			po1.setAcknowledgedate(dtf.format(now));
		po1.setStatus(status);
		POrderRepository.save(po1);
		  return ResponseEntity.created(null).body(true);
		
	}

	public ResponseEntity<?> UpdatePoApprove(Integer id, Integer aprid, Integer status) {
		Optional<PurchaseOrder> po=POrderRepository.findById(id);
		PurchaseOrder po1=po.get();
           po1.setApprovedid(aprid);
		   DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");  
		   LocalDateTime now = LocalDateTime.now();  
		   System.out.println(dtf.format(now));  
		   po1.setApproveddate(dtf.format(now));
			po1.setStatus(status);
			POrderRepository.save(po1);
			  return ResponseEntity.created(null).body(true);
	...
	public ResponseEntity<?> UpdateStatusRejected(Integer id, Integer rejid, Integer status) {
		...
		PurchaseOrder pr1=po.get();
		   ...
		   pr1.setRejecteddate(dtf.format(now));
			...
			POrderRepository.save(pr1);
			  return ResponseEntity.created(null).body(true);
	}

	public ResponseEntity<?> ViewPurchaseOrder(Integer compid, Integer brnchid, Integer loc, Integer locrefid) {
		List<PurchaseOrder> po = POrderRepository.findByCompanyrefidAndBranchidAndLocnameAndLocrefid(compid, brnchid, loc,
				locrefid);
		return ResponseEntity.created(null).body(po);
	}

	public ResponseEntity<?> viewPurchaseOrderid(Integer id) {
		Optional<PurchaseOrder> po = POrderRepository.findById(id);
		return ResponseEntity.created(null).body(po);
	}

	public ResponseEntity<?> viewPoProductid(Integer porefid) {
		List<PoProduct> po = poprorepo.findByPorefid(porefid);
		return ResponseEntity.created(null).body(po);
	}

	public ResponseEntity<?> viewPurchaseorderbyid(Integer id) {
		Optional<PoProduct> po = poprorepo.findById(id);
		return ResponseEntity.created(null).body(po);
		
	}

	public ResponseEntity<?> getpurchaseorder(Integer poid) {
		List po = poprorepo.getPono(poid);
		return ResponseEntity.created(null).body(po);
	}

	public ResponseEntity<?> GetpurchaseorderProductid(Integer poid) {
		Optional<PoProduct> po = poprorepo.findById(poid);
		return ResponseEntity.created(null).body(po);
	}

	public ResponseEntity<?> deletePurchaseorderproduct(Integer id) {
		PoProduct po= poprorepo.findById(id).get();
		po.setStatus(5);
		poprorepo.save(po);
		return ResponseEntity.created(null).body(true);
	}

	public ResponseEntity<?> viewid(Integer id) {
		Optional<PoProduct> po = poprorepo.findById(id);
		return ResponseEntity.created(null).body(po);
	}

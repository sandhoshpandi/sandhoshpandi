...
public interface DistProdRepository extends JpaRepository<DistributortProd, Integer> {
	...
	// Boopalan 270619
	@Query(value = "SELECT DistributorID,DistributorName  FROM medc_distributor.medc_distributorinformation     where status != 5 and companyrefid = ?1  ", nativeQuery = true)
	...
	//Mahalakshmi
	@Query(value = "SELECT pm.PCompanyID,pm.PCompanyName,di.DistributorName,di.DistributorID FROM medc_pharmacompany.medc_pharmacompanies pm INNER JOIN medc_storereg.medc_shopinformation sh ON sh.country = pm.CountryofOrigin AND sh.ShopID = pm.LocRefID  INNER JOIN medc_distributor.medc_distphcompany da on da.phcompanyrefid = pm.PCompanyID INNER JOIN medc_distributor.medc_distributorinformation di on da.DistRefID = di.DistributorID WHERE di.DistributorID=?1 and pm.Status != 5 ", nativeQuery = true)
	List findBydistributorid(Integer distributorid);
